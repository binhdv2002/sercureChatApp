...
socket.on("connect", () => {
    ...
    socket.on("by-server-to-receiver-new-invitation", (data) => {
        ...
        if (sharedKeyInfo) {
            ...
            console.log("Shared key from local storage: " + info.sharedKey, "   -Shared key: " + combinedName);
        }
        ...
        if (!sharedKeyInfo) {
            ...
            //     iterations: 10000,
            // }
            const key = CryptoJS.MD5(passwordWordArray).toString(CryptoJS.enc.Hex);
            sharedKey = key;
            ...
            console.log(`Shared Secret Key of ${receiverName}: ${sharedKey}   ,  ${password}`);
        }
    });
    ...
    socket.on("by-server-to-sender-invitation-accepted", (data) => {
        ...
        if (!sharedKeyInfo) {
            ...
            localStorage.setItem(combinedName, JSON.stringify(info));
            console.log(`Shared Secret Key of ${senderName}: ${sharedKey}  ,  ${password}`);
        }
    });
